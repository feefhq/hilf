...
function rowToCard(row, index) {
  const [germanWord, article, partOfSpeech, englishTranslation, germanExample, englishExample, category] = row;
  ...
  const answer = (article && article.trim()) ? `${article.trim()} ${(germanWord || '').trim()}`.trim() : (germanWord || '').trim();
  ...
  const card = {
    ...
    answer,
    level: 'A1',
    type: partOfSpeech && /noun|verb|adv|adj/i.test(partOfSpeech) ? 'word' : 'phrase',
  };
...
`;
  ...
  const outPath = path.join(process.cwd(), 'src', 'vocabulary', 'a1', 'words.ts');
